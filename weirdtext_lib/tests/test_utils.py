...
from weirdtext_lib.excepions import (
    SeparatorAmountException,
    SeparatorLocalisationException,
)


@pytest.mark.parametrize(
    "to_be_shuffled, expected",
    [("biiig", "biiig"), ("Yuuup", "Yuuup"), ("ooooooO", "ooooooO")],
)
...
@pytest.mark.parametrize(
    "to_be_shuffled", [("qwerty"), ("four"), ("equality"), ("shuffle")]
)
def test_shuffle(to_be_shuffled):
    assert shuffle_word(to_be_shuffled) != to_be_shuffled


def test_preprocess_encoding(
    base_coded_sentence, preprocessed_encoded_text, list_of_original_words, separator
):
